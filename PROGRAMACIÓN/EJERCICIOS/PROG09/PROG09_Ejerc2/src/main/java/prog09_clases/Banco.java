...
public class Banco{
    ...
    /**
     * Guarda la información de las cuentas
     */ 
    public void guardarInfo(){        
        
        try {
            ObjectOutputStream escribirObjeto = new ObjectOutputStream(new FileOutputStream("datoscuentasbancarias.dat"));
            // Se guarda cada cuenta que hay en estos momentos en el banco
            for(CuentaBancaria cuenta: banco){
                escribirObjeto.writeObject(cuenta);
            }            
    ...
    /**
     * Recupera la información de las cuentas guardadas con anterioridad
     */
    public void recuperarInfo(){
        
        try {
            ObjectInputStream leerObjeto = new ObjectInputStream(new FileInputStream("datoscuentasbancarias.dat"));
            // Va a seguir funcionando hasta que se termine el archivo
            while(true){
                try{
                    CuentaBancaria cuenta = (CuentaBancaria) leerObjeto.readObject();
                    this.abrirCuenta(cuenta);
                }catch(EOFException ex){
                    // Se captura EOFException cuando se alzanza el final del archivo
    ...
    /**
     * Genera un archivo con la información de los clientes y su iban
     * @return true si se realiza correctamente, false si no
     */
    public boolean listarClientes(){
        
        try {
            BufferedWriter listado = new BufferedWriter(new FileWriter("ListadoClientesCCC.txt"));
            for (CuentaBancaria cuenta: banco){
                listado.write(cuenta.getTitular() + ", IBAN= "+cuenta.getIban());
                listado.newLine();
            }
            listado.write("Número de cuentas: " + banco.size());
            listado.close();
